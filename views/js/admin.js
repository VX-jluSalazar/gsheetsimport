document.addEventListener('DOMContentLoaded', function () {
  ...
  async function callAjax(url, action) {
    ...
    try {
      ...
      throw new Error(
        'Respuesta del servidor no válida (se esperaba JSON). HTTP ' +
        response.status +
        (preview ? ': ' + preview : '.')
      );
    }
    ...
    if (!response.ok || !json.success) {
      throw new Error(json.message || 'Error AJAX inesperado.');
    }
  ...
  async function processLoop(url) {
    ...
    if (result.pending > 0) {
      setStatus('Procesando productos... Pendientes: ' + result.pending, 'info');
      return processLoop(url);
    }

    setStatus('Sincronización de productos completada.', 'success');
  ...
  if (fetchButton) {
    fetchButton.addEventListener('click', async function () {
      try {
        ...
        setStatus('Cargando productos desde Google Sheets...', 'info');
        const result = await callAjax(url, 'FetchSheet');

        updateSummary(result.summary);
        updateProgress(result.summary);

        setStatus('Productos cargados a staging: ' + (result.total_rows || 0), 'success');
        window.location.reload();
      } catch (error) {
  ...
  if (processButton) {
    processButton.addEventListener('click', async function () {
      try {
        ...
        setStatus('Iniciando creación/actualización de productos...', 'info');
  ...
  if (pushButton) {
    pushButton.addEventListener('click', async function () {
      try {
        ...
        setStatus('Sincronizando productos de PrestaShop hacia Google Sheets...', 'info');
        const result = await callAjax(url, 'PushSheet');

        setStatus(
          'Google Sheets actualizado. Actualizados: ' + (result.updated || 0) +
          ', añadidos: ' + (result.appended || 0) +
